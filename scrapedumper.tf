...
resource "docker_service" "scrapedumper" {
  ...
  task_spec {
    container_spec {
      image = "smartatransit/scrapedumper:${split(data.docker_registry_image.scrapedumper.sha256_digest)}"
...
resource "docker_service" "scrapereaper" {
  ...
  task_spec {
    container_spec {
      image = "smartatransit/scrapereaper:${split(data.docker_registry_image.scrapereaper.sha256_digest)[0]}"
